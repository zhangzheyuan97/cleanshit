...
(function ($) {
    ...
    function TagsInput(element, options) {
        ...
        this.$input = $('<input type="text" data-roles="mui-validatebox" data-options="validType:\'maxLength[20]\'" placeholder="' + this.placeholderText + '"/>').appendTo(this.$container);
    ...
    TagsInput.prototype = {
        ...
        add: function (item, dontPushVal, options) {
            ...
            if (undefined != focusedTag) {
                focusedTag.className = "sTags-selected";
            }
            // Trim value
            ...
            var existing = $.grep(self.itemsArray, function (item) {
                return self.options.itemTitle(item) === itemTitle;
            })[0];
            ...
            if (dontPushVal) {
                $tag = $('<span class="badge ' + htmlEncode(tagClass) + (itemTitle !== null ? ('" title="' + itemTitle) : '') + '"  tagid="' + focusedTag.getAttribute('tagid') + '" tagname="' + focusedTag.getAttribute('tagname') + '" tagInitial="' + focusedTag.getAttribute('tagInitial') + '" tagType="' + focusedTag.getAttribute('tagType') + '">' + htmlEncode(itemText) + '<span data-role="remove" style="margin-right:8px "></span></span>');
            } else {
                $tag = $('<span class="badge ' + htmlEncode(tagClass) + (itemTitle !== null ? ('" title="' + itemTitle) : '') + '"  tagid="" tagname="" tagInitial="" tagType="">' + htmlEncode(itemText) + '<span data-role="remove" style="margin-right:8px "></span></span>');
            }
            $tag.data('item', item);
        ...
        addFromObject: function (item, dontPushVal, options) {
            ...
            if (dontPushVal) {
                $tag = $('<span class="badge ' + htmlEncode(tagClass) + (itemTitle !== null ? ('" title="' + itemTitle) : '') + '"  tagid="' + options.id + '" tagname="' + options.tag + '" tagInitial="' + options.tagInitial + '" tagType="' + options.tagType + '">' + htmlEncode(itemText) + '<span data-role="remove" style="margin-right:8px "></span></span>');
            } else {
                $tag = $('<span class="badge ' + htmlEncode(tagClass) + (itemTitle !== null ? ('" title="' + itemTitle) : '') + '"  tagid="" tagname="" tagInitial="" tagType="">' + htmlEncode(itemText) + '<span data-role="remove" style="margin-right:8px "></span></span>');
            }
            $tag.data('item', item);
        ...
        build: function (options) {
            ...
            if (self.options.addOnBlur && self.options.freeInput) {
                self.$input.on('focusout', $.proxy(function (event) {
                    ...
                    var maxLengthReached = self.options.maxChars && self.$input.val().length > self.options.maxChars;
                    if (!maxLengthReached && $('.typeahead, .twitter-typeahead', self.$container).length === 0) {
            ...
            self.$container.on({
                focusin: function () {
                    ...
                    self.$input.attr('style','width:120px');
                },
                focusout: function () {
                    var maxLengthReached = self.options.maxChars && self.$input.val().length > self.options.maxChars;
                    if(!maxLengthReached){
                    ...
                    case 8:
                        if (doGetCaretPosition($input[0]) === 0) {
                            ...
                            if (prev.length) {
                                ...
                                let title = prev.data('item');
                                self.remove(prev.data('item'));
                                //级联取消待选框的选中状态

                                if (undefined != title && '' != title) {
                                    let selectedTags = $('.sTags .sTags-selected');
                                    for (let i = 0; i < selectedTags.size(); i++) {
                                        let selectedTagTitle = selectedTags.get(i).children[0].getAttribute('title');
                                        if (title == selectedTagTitle) {
                                            selectedTags.get(i).setAttribute('class', '');
                                            break;
                                        }
                                    }
                                }
            ...
            self.$container.on('click', '[data-role=remove]', $.proxy(function (event) {
                ...
                //级联取消待选框的选中状态
                let title = $(event.target).parent().attr('title');
                if (undefined != title && '' != title) {
                    let selectedTags = $('.sTags .sTags-selected');
                    for (let i = 0; i < selectedTags.size(); i++) {
                        let selectedTagTitle = selectedTags.get(i).children[0].getAttribute('title');
        ...
        var list = function (target) {//列出数据 target目标div指 定义选择数据的div
            ...
            if (o.data.length > 0) {//如果有标签数据
                ...
                for (var i in o.data) {//循环标签数据
                    ...
                    E.click(function () {
                        focusedTag = this;
                        if (this_.prop("tagName") == "INPUT" && this.className == o.sTagsSelectedCss) {
                            tagSelectedInput.remove(this.getAttribute('tagname'), true);
                        } else {
                            tagSelectedInput.add(this.getAttribute('tagname'), true);
                        }
                    })
